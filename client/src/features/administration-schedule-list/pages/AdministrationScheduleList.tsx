...
import { getAdministrationScheduleListAsync } from "../../../services/AdministrationScheduleListService";
import { IAdministrationScheduleDetailListResponse } from "../../../models/interface/IAdministrationScheduleListResponse";
import {
  IAdministrationScheduleDetailList,
  IAdministrationScheduleList,
} from "../../../models/interface/IAdministrationScheduleList";
import { useSelector } from "react-redux";
import { RootState } from "../../../redux/store";
import { AppConstant } from "../../../shared/constants/AppConstant";

const AdministrationScheduleList = (props: any) => {
  ...
  const [admininstrationScheduleList, setAdmininstrationScheduleList] =
    useState<IAdministrationScheduleList[]>([]);
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length >= 2) {
      ...
      pagination.pageNumber = parseInt(
        sessionStorage.getItem("administrationScheduleListPageNumber")!
      );
      let newPagination = pagination;
      setPagination(newPagination);
      getAdministrationScheduleList(
  ...
  const getAdministrationScheduleList = useCallback(
    async (
      pageNumber: number,
      ...
      const response: IAdministrationScheduleDetailListResponse =
        await getAdministrationScheduleListAsync(
          pageNum,
          libraryName.search,
      ...
      const { administrationScheduleList, pagination: responsePagination } =
        response || {};

      if (administrationScheduleList !== undefined) {
        const sortedLibraries = sortAdminsistartionScheduleList(
          administrationScheduleList,
          isSortDescending
        );
        setAdmininstrationScheduleList(sortedLibraries);
        ...
        setAdmininstrationScheduleList([]);
      }
  ...
  const sortAdminsistartionScheduleList = (
    admininstrationScheduleList: IAdministrationScheduleDetailList[],
    isSortDescending?: boolean
  ) => {
    return admininstrationScheduleList.sort(
      (
        a: IAdministrationScheduleDetailList,
        b: IAdministrationScheduleDetailList
      ) => {
  ...
  const navigateToAdministrationScheduleList = (id: number) => {
    const selectedScheduleObj = admininstrationScheduleList.find(x=>x.id === id);
    sessionStorage.setItem(
      "selectedAdministrationScheduleDetails",
  ...
  if (props?.sendToChild === "tab1" && pagination.pageNumber !== 1) {
  ...
  // navigate to Administration Schedule Lists/summary page
  const navigateToFrequencyAdminstration = (id: number) => {
    navigate(`${basePath}/frequency-adminstration`);
  };
      ...
      {isLoading ? (
        ...
        <>
          <div className="row" style={{marginTop : "-21px"}}>
            <div className="col-8">
              <h3>Administration Schedule List</h3>
            </div>
          ...
          <div className="row">
            <div className="col-12">
                ...
                <table className="table table-sm table-striped dark-border-header table-hover">
                  <thead>
                    <tr>
                      <th className="dark-border-header">
                        List Name
                        {admininstrationScheduleList?.length === 0 ? (
                          <i
                            data-testid="sort-library-name"
                  ...
                  <tbody>
                    {admininstrationScheduleList?.length === 0 ? (
                      ...
                      admininstrationScheduleList?.map(
                        (itm: IAdministrationScheduleList, index) => (
                          <tr key={itm.id + itm.description + index}>
                            <td>
                              <button
                                ...
                                onClick={() =>
                                  navigateToAdministrationScheduleList(itm?.id)
                                }
                              >
                                {itm.description}
                              </button>
              ...
              {admininstrationScheduleList?.length > 0 && showPagination ? (
...
export default AdministrationScheduleList;
